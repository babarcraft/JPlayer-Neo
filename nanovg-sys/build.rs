...
fn build_library(backend_macro: &str) {
    ...
    config.define("GL_GLEXT_PROTOTYPES", None);
...
fn main() {
    // let mut enabled = vec![];
    //
    // for f in ["GL3", "GL2", "GLES3", "GLES2"] {
    //     if env::var(format!("CARGO_FEATURE_{}", f)).is_ok() {
    //         enabled.push(f);
    //     }
    // }
    //
    // if enabled.is_empty() {
    //     panic!(
    //         "No backend feature enabled. Enable one of: gl3, gl2, gles3, gles2"
    //     );
    // }
    //
    // let f = enabled[0];
    // let backend_macro = format!("NANOVG_{}_IMPLEMENTATION", f);
    //
    ...
    build_library("NANOVG_GL3_IMPLEMENTATION");
}
